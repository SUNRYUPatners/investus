...
const COPY: Record<StockMarket, { badge: string; title: string; sub: string; status: string; icon: string }> = {
  us: {
    badge: "● 장중 LIVE",
    title: "미국주식 장중",
...
export function MarketOpenBanner({ market }: { market: MarketId }) {
  ...
  return (
    <>
      ...
      >
        <div
          className="relative px-4 py-3 lg:px-5 lg:py-4 flex items-center gap-3 overflow-hidden"
        ...
        >
          ...
          />
          <div
            className="relative w-11 h-11 lg:w-[52px] lg:h-[52px] rounded-xl flex-shrink-0 flex items-center justify-center"
            style={{
          ...
          <span
            className="hidden sm:inline-flex text-[10px] font-bold px-2.5 py-1 rounded-full flex-shrink-0"
            style={{ background: "rgba(var(--up-rgb),0.15)", color: "var(--up)" }}
          >
            {copy.status}
          </span>
        </div>
