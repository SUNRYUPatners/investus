...
{ file:'tsla-fsd-scale-only', symbol:'TSLA', layout:'tpl', seed:'seed-1103',
  ...
  ko:{title:'규모 있는 자율주행을 실제로 하는 회사는 테슬라뿐이라는 주장이 나왔습니다',heroIcon:'🚕',heroBig:'규모',heroSub:'규모 있는 자율주행은 시험차 몇 대가 아니라, 많은 차가 매일 실제 도로에서 소프트웨어로 운행하는 상태를 뜻합니다. 이번 주장은 그 기준에서 테슬라만 해당한다는 쪽입니다.',
    cards:[{icon:'🚕',big:'로보택시',mid:'오스틴·달러스',sub:'운행 구역'},{icon:'📊',big:'FSD',mid:'감독 주행',sub:'마일 축적'},{icon:'⚖️',big:'검증',mid:'유료·무인',sub:'정의가 핵심'}],
  ...
  en:{title:'Only Tesla does self-driving at scale, a claim says',heroIcon:'🚕',heroBig:'SCALE',heroSub:'At scale means many cars daily on public roads under software control, not a handful of test cars. The claim is that only Tesla meets that bar.',
    cards:[{icon:'🚕',big:'Robotaxi',mid:'Austin·Dallas',sub:'Service zones'},{icon:'📊',big:'FSD',mid:'Supervised',sub:'Mile build'},{icon:'⚖️',big:'Proof',mid:'Paid·driverless',sub:'Definitions'}],
    quote:'"A claim says Tesla is the only company doing self-driving at scale. Scale means commercial-like fleet size and hours, not a few pilots. Waymo and Zoox compete under different cities and rules. Uniqueness depends on driverless, paid service, and miles. Separate the claim from official stats."',
    noteHead:'Why this matters',noteSub:'A monopoly narrative supports Tesla’s premium. Narrow the definition to paid miles and cities. Rivals expanding paid driverless service can weaken the frame.',footer:'TSLA · FSD scale',brand:BE}},
...
{ file:'avgo-nvda-cds-record', symbol:'AVGO', layout:'tpl', seed:'seed-1108',
  ...
  ko:{title:'브로드컴·엔비디아 CDS가 8월 차트 기준 사상 최고 구간에 있습니다',heroIcon:'📉',heroBig:'CDS',heroSub:'2026년 8월 차트에서 브로드컴 약 126.2bp, 엔비디아 약 86.7bp로 사상 가장 넓은 구간입니다. CDS는 회사 부도 위험에 대한 보험료에 가깝습니다.',
    cards:[{icon:'📉',big:'126.2',mid:'브로드컴',sub:'bp 스프레드'},{icon:'📉',big:'86.7',mid:'엔비디아',sub:'bp 스프레드'},{icon:'📈',big:'사상 최고',mid:'8월 차트',sub:'가장 넓은 구간'}],
    quote:'"신용부도스와프(CDS) 스프레드가 넓어졌다는 것은 시장이 신용 위험에 더 높은 프리미엄을 요구한다는 뜻입니다. 주가가 강해도 CDS만 먼저 경계할 수 있습니다. AI·반도체 낙관과 신용 헤지가 동시에 존재할 수 있으니, 실적·회사채와 같이 보시면 됩니다. 하루 급등과 수개월 추세는 의미가 다릅니다."',
    noteHead:'왜 중요한가',noteSub:'주가와 CDS는 항상 같이 움직이지 않습니다. 실적·가이던스가 신용을 안정시키는지, 일시 헤지인지 추적하세요.',footer:'AVGO·NVDA · CDS',brand:BK},
  en:{title:'Broadcom and Nvidia CDS at all-time wides on August charts',heroIcon:'📉',heroBig:'CDS',heroSub:'August 2026 charts: Broadcom about 126.2bp and Nvidia about 86.7bp, both at all-time wides. CDS is like an insurance premium on default risk.',
    cards:[{icon:'📉',big:'126.2',mid:'Broadcom',sub:'bp spread'},{icon:'📉',big:'86.7',mid:'Nvidia',sub:'bp spread'},{icon:'📈',big:'Record',mid:'August chart',sub:'All-time wide'}],
    quote:'"A wider CDS spread means the market is charging a higher premium for credit risk. Equities can stay strong while CDS flashes caution. AI optimism and credit hedges can coexist, so read earnings and corporate bonds together. A one-day spike is not the same as a multi-month trend."',
    noteHead:'Why this matters',noteSub:'Stocks and CDS do not always move together. Track whether earnings stabilize credit, or whether this is temporary hedging.',footer:'AVGO·NVDA · CDS',brand:BE}},

{ file:'macro-treasury-bond-intervention', symbol:'MACRO', layout:'tpl', seed:'seed-1109',
  ...
  ko:{title:'미국 재무부의 국채 시장 개입·환매 이야기가 다시 부각됐습니다',heroIcon:'🏦',heroBig:'환매',heroSub:'40억 달러 이상 국채 환매, 약 9,500억 달러 일반계정 이야기가 다시 나왔습니다. 국채 수급·유동성 기대가 장기금리에 영향을 줄 수 있습니다.',
    cards:[{icon:'💵',big:'$4B+',mid:'환매 규모',sub:'이야기 기준'},{icon:'🏦',big:'$950B',mid:'일반계정',sub:'재무부 현금'},{icon:'📉',big:'장기금리',mid:'수급·유동성',sub:'할인율 연결'}],
    quote:'"재무부가 이미 발행한 국채를 다시 사들이는 환매는 수급을 완화하는 도구입니다. 일반계정은 재무부가 연준에 맡긴 현금 통장입니다. 잔고가 크면 시중 유동성에 영향을 줄 수 있습니다. 발언과 실제 집행 규모는 다를 수 있으니, 주간 환매·잔고·10년·30년 금리를 숫자로 확인하세요. 헤드라인만으로 금리 하락을 확정하면 위험합니다."',
    noteHead:'왜 중요한가',noteSub:'장기금리는 주식·부동산 할인율과 한 묶음입니다. 실제 환매와 일반계정 잔고를 확인하시고, 물가·국채 발행 물량과 같이 보시면 됩니다.',footer:'MACRO · 국채 환매',brand:BK},
  en:{title:'Treasury buyback and intervention talk is back in focus',heroIcon:'🏦',heroBig:'Buyback',heroSub:'Talk of $4B+ Treasury buybacks and a ~$950B general account is back. Supply, liquidity, and long yields can move together.',
    cards:[{icon:'💵',big:'$4B+',mid:'Buybacks',sub:'Talked size'},{icon:'🏦',big:'$950B',mid:'TGA cash',sub:'Treasury account'},{icon:'📉',big:'Yields',mid:'Supply·liquidity',sub:'Discount rates'}],
    quote:'"A Treasury buyback means the government buys its own bonds back in the market to ease supply. The general account is Treasury cash at the Fed, and a large balance can absorb liquidity. Rhetoric and actual execution can differ, so check weekly buybacks, balances, and 10Y/30Y yields. Do not treat a headline as a guaranteed rate drop."',
    noteHead:'Why this matters',noteSub:'Long yields sit with equity and housing discount rates. Verify actual buybacks and TGA balances alongside inflation and issuance.',footer:'MACRO · Treasury buyback',brand:BE}},
...
{ file:'nvda-chip-smuggling-china', symbol:'NVDA', layout:'tpl', seed:'seed-1113',
  category:'종목분석', categoryColor:'blue', subject:'엔비디아', tickers:['NVDA'],
  ...
  ko:{title:'엔비디아 직원이 고급 칩을 중국으로 밀반출한 혐의로 기소됐다는 보도가 나왔습니다',heroIcon:'🛂',heroBig:'수출',heroSub:'고급 AI 칩을 중국으로 밀반출한 혐의로 직원이 기소됐다는 보도입니다. 수출 통제와 내부 준수, 우회 수요 이슈로 읽힙니다.',
    cards:[{icon:'🛂',big:'수출 통제',mid:'대중국 규칙',sub:'허가 제한'},{icon:'⚖️',big:'기소',mid:'직원 1건',sub:'개인 사건'},{icon:'🇨🇳',big:'중국',mid:'수요·우회',sub:'리스크'}],
    quote:'"엔비디아 직원이 고급 칩을 중국으로 밀반출한 혐의로 기소됐다는 보도가 나왔습니다. 미국은 AI 칩 중국 수출을 제한합니다. 개인 사건과 회사 정책 위반은 구분해야 하지만, 시장은 내부 통제·규제 감독·우회 수요 이야기와 연결해 읽습니다. 합법 매출과 암시장 수요를 나눠 보셔야 합니다. 회사 공식 입장과 법원 서류를 확인하는 것이 다음 단계입니다."',
  ...
  en:{title:'Nvidia employee charged with smuggling advanced chips to China',heroIcon:'🛂',heroBig:'Export',heroSub:'Reports say an Nvidia employee was charged with smuggling advanced AI chips to China. Export controls, compliance, and diversion risk are in focus.',
    cards:[{icon:'🛂',big:'Controls',mid:'China rules',sub:'License limits'},{icon:'⚖️',big:'Charge',mid:'One employee',sub:'Personal case'},{icon:'🇨🇳',big:'China',mid:'Demand·diversion',sub:'Tail risk'}],
    quote:'"Reports say an Nvidia employee was charged with smuggling advanced chips to China. The U.S. restricts high-end AI chip exports. A personal case is not the same as company policy, but markets still read compliance and diversion risk. Separate legal sales from grey-market demand, and wait for company statements and filings."',
    noteHead:'Why this matters',noteSub:'China demand and tighter controls are tail risks. Check the company statement and revenue mix before changing a long-term view on one headline.',footer:'NVDA · chip smuggling',brand:BE}},
];
