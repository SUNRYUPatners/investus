...
종토방(wallPosts) · 애널 피드(analystPosts) 내용을 **스크린샷 원문 및 SVG 이미지와 1:1 대조**한다.

#### 체크 항목
- 수치·퍼센트·가격·날짜 → 스크린샷 숫자와 정확히 일치하는지
- 회사명·종목·사람 이름 → 오탈자·혼동 없는지
- 방향성(상승/하락, 매수/매도, 승인/부결) → 원문 방향과 동일한지
- 없는 사실 추가 여부 → 스크린샷에 없는 내용 임의 창작 금지

#### 체크 방법
```
# 작성한 wallPosts / analystPosts 내용에서 수치·이름·방향 추출
# → 해당 날짜 SVG 파일 또는 스크린샷과 대조
# → 불일치 발견 즉시 수정 후 재확인
```
