...
3. **투자시사점은 본문에만 — SVG 이미지에 절대 넣지 않는다 (반복 위반 금지)**
   - 모든 개별 리포트 `body` / `bodyEn`에 `■ 투자시사점` (EN: `■ Investment Implications`) — **여기 필수**
   - **SVG 안에는 “투자 시사점” / “Investment Implications” 텍스트·박스를 절대 넣지 않는다.** 이미지는 팩트(숫자·카드)만, 해석/시사점은 글로만.
   - 과거 위반 사례(2026-07-15 최초본)에서 SVG마다 시사점 박스가 들어가 있었음 → 전부 제거함. 다시 넣지 말 것.

4. **SVG는 2026-07-02 스타일 — 폰트 크기 구체 기준 + 빈공간 채우기**
   - 레퍼런스: `public/charts/*20260702*.svg` (텍스트 절대 작게 만들지 말 것)
   - 제목: 22~28px · 히어로 숫자: 84~130px · 히어로 라벨: 24~26px · 히어로 서브: 18~20px
   - 카드: 라벨 18~20px(회색) / **값은 34~56px로 크게(카드의 주인공)** / 서브 16~18px — 절대 14~15px로 값을 쓰지 않는다
   - 상세 배경·긴 시사점 → 본문. SVG에는 **한 줄짜리 맥락**까지는 OK. Source 표기는 박스 없이 얇은 캡션 한 줄로.
   - **글씨를 키운 후 생기는 빈 공간은 반드시 채운다** — 카드 아래 "핵심 배경" 박스(팩트 2줄, 22px 안팎)를 추가해 캔버스 하단까지 densely 채울 것. 카드 3~4개 + 빈 공백만 있고 하단 300px+가 비어있으면 실패. (7/2 레퍼런스처럼 quote box·YoY row 등으로 끝까지 채워야 함)
   - 단, 채우는 내용은 **팩트/배경**만 — "투자 시사점" 문구·해석은 절대 넣지 않는다 (원칙 3 위반)
...
| **SVG에 “투자 시사점” 박스 넣기** | 시사점은 **본문에만**, SVG는 팩트만 |
| 카드 값 폰트 14~15px (너무 작음) | 카드 값 34~56px (크게, 카드의 주인공) |
