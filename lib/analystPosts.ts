...
export const MOCK_ANALYST_POSTS: AnalystMockPost[] = [
  ...
  },
  {
    ...
    content: "의견과 일정을 구분해 둡니다. 스페이스X 장외가가 전일보다 2.60% 오른 154.81달러로, 한 달 고점 156.87달러에 가깝습니다. 시가총액은 하루에 약 510억 달러 늘었습니다. 실패한 스타트업 데이터를 인공지능 학습에 쓰려는 논의가 같은 화면에 올랐습니다.",
  ...
  },
  {
    ...
    content: "스페이스X 카드입니다. 스타십 14차 비행이 9월 22일로 확정됐다는 소식에 스페이스X 장외 주가가 5.15% 급등해 150.88달러로 마감했습니다. 이번 비행은 처음으로 완전한 지구 궤도에 들어가 스타링크 V3 위성을 배치하는 시도로, 위성 26기가 팰컨9 로켓 10번어치의 통신 용량을 실어나릅니다. 머스크는 발사당 10억 달러 매출 시나리오에 대해 불가능하지 않다고 답했습니다.",
  ...
  // ── 2026-09-16 신규 (20개 · 존댓말 · 구조 혼합) ──────────────────────
  {
    ...
    content: "오늘 화면의 숫자부터입니다. 예측시장 폴리마켓은 일론 머스크가 2027년 말까지 테슬라와 스페이스X의 합병을 공식 발표할 확률을 52%로 매겼습니다. 머스크는 올인서밋 행사에서 합병 여부를 묻는 질문에 \"질문 참 좋네요, 이렇게 협업이 많은데 말이죠\"라며 즉답을 피했습니다. 같은 날 스페이스X 장외가는 143.49달러로 3.15% 하락했습니다.",
...
export const MOCK_ANALYST_COMMENTS: Record<number, AnalystMockComment[]> = {
  ...
  [-1317]: [
    { alias: "삼성동 올빼미 #19", content: "장외 호가가 얇아 하루 시총 출렁임이 큽니다.", created_at: "2026-09-18T00:30:04:00.000Z" },
  ],
  [-1318]: [
