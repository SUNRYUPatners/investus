...
export const MOCK_ANALYST_POSTS: AnalystMockPost[] = [
  ...
  },
  {
    ...
    symbol: "INTC",
    content:
      "미국 정부 INTC 매도 내역 뜯어봤는데 — $21.61에 팔았어. 사들인 가격이 $31.04이니까 주당 -$9.43, 총 -$4B 손실이야. 정부가 CHIPS Act로 지원하면서 동시에 주식 매도한 거 자체가 메시지임. 내부에서 '이 회사 회복에 시간이 너무 걸린다'는 판단이 있었던 거 아닐까? 반론도 있어 — 비상장 지분 정리 / 정치적 이해충돌 회피 목적일 수 있음. 하지만 $4B 손실 확정하고도 파는 행위는 장기 보유 의지가 없다는 신호임. 18A 공정 2027 양산이 변수지만 그 전까지 촉매가 없어. 역발상 관심주로는 보되 지금 진입 서두를 필요 없다고 봄.",
    likes: 334,
    comments: 3,
...
export const MOCK_ANALYST_COMMENTS: Record<number, AnalystMockComment[]> = {
  ...
  [-96]: [
    { alias: "신촌 늑대 #33", content: "정부가 산 건 CHIPS Act 지원 때 지분 취득한 거임. 근데 손실 보면서 판다는 건 그냥 정치적 포지션 정리일 수 있어. 악재로 읽기보다 중립으로 보는 게 맞을 수도.", created_at: "2026-06-19T08:46:00.000Z" },
    { alias: "여의도 올빼미 #44", content: "18A 공정이 TSMC 3nm 수준이면 실제로 경쟁력 있어. 2027 양산 성공 여부가 INTC 재평가 트리거임. 그 전까지 기다리는 포지션이 맞는 거 같음.", created_at: "2026-06-19T08:42:00.000Z" },
    { alias: "목동 황소 #23", content: "역발상으로 보면 정부가 팔았으니까 '팔 사람 다 팔았다'는 해석도 가능해. 18A 양산 소식 나오면 단기 급등 나올 수 있어서 소량 관심 가져볼 만한 종목임.", created_at: "2026-06-19T08:38:00.000Z" },
  ],
  [-95]: [
  ...
  [-87]: [
    ...
    { alias: "강남 팔콘 #62", content: "스페인 6/30 오픈이면 남유럽 3국 도미노 가능성 있어. 이탈리아·포르투갈이 스페인 레퍼런스 보고 빠르게 따라오는 패턴이 이민 정책이나 EV 보조금에서도 나왔었음.", created_at: "2026-06-19T08:06:00.000Z" },
