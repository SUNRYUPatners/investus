...
export const MOCK_ANALYST_POSTS: AnalystMockPost[] = [
  ...
  },
  {
    ...
    symbol: null,
    content:
      "사우디 Arab Light Crude 할인 전환 — 매크로 임팩트 분석. 1) Saudi Arabia Arab Light Crude 현재 -1.50. 올해 5월 31일 고점 +19.50에서 2020년 이후 처음 마이너스 구간 진입이야. 2020년 저점이 -7.30이었는데 방향성 확인 필요해. 2) OPEC+ 증산+글로벌 수요 둔화 = 공급 과잉이야. 사우디가 할인 판매까지 하는 건 시장 점유율 방어 전략이야. 가격 경쟁 돌입 신호야. 3) 에너지 인플레이션 압력 완화 → CPI 에너지 항목 하락 → 연준 금리 인하 논거 강화. 성장주 수혜 구조야. 4) EV 채택 가속과 EV 수요 둔화 논쟁에서 저유가가 혼재 변수야. 에너지 기업 매도, 성장주 매수 리밸런싱 예상. NEUTRAL.",
    likes: 876,
    comments: 2,
  ...
  },
  {
    ...
    symbol: "SPCX",
    content:
      "SpaceX NASDAQ-100 편입 + WSJ 보도 — $800B AUM 강제 매수 분석. 1) WSJ 'SpaceX Is About to Join the Nasdaq-100' 보도. 인베스코 QQQ 포함 $8000억 AUM ETF가 월요일 종가 기준 강제 매수 집행이야. 패시브 자금이 가격 불문 시장가 집행하는 거야. 2) SPCX +1.89% 편입 당일. 강제 집행일까지 추가 상승 여지 있어. 3) 기관 투자자 자격 완성 = 연기금·대학 기금 편입 가능해짐. 장기 수요 기반이 구조적으로 달라지는 이벤트야. 4) Starlink 성장+Falcon 재사용+Starship 상업화 모두 진행 중. NASDAQ 편입으로 기관 커버리지 확대. BUY.",
    likes: 2134,
    comments: 5,
  ...
  },
  {
    ...
    symbol: "TSLA",
    content:
      "Tesla 임팩트 리포트 '25 p.77 — FSD 안전 통계 분석. 1) ~900만 대, 110억+ 마일 데이터 기반. 미국 국가 평균 대비 대형 충돌 8배 감소, 소형 충돌 7배 감소, 도로이탈 6배 감소. 2) 110억 마일이 경쟁사랑 비교 불가 스케일이야. 웨이모 누적 수백만 마일이랑 차원이 달라. 이 데이터로 규제 기관 설득 불가능한 반박이야. 3) 보험 회사 설득 자료로 최적이야. 대형 충돌 8배 감소면 보험료 인하 협상 가능해. FSD 구독 경제성 추가 개선 구조야. 4) 900만 대 플릿 러닝 = 데이터 해자. 경쟁사는 절대 따라잡을 수 없는 구조. STRONG BUY.",
    likes: 2312,
    comments: 6,
  ...
  },
  {
    ...
    symbol: "TSLA",
    content:
      "Tesla 임팩트 리포트 '25 — OTA 300+ 기능 + 리콜 98% 해결 분석. 1) 2025년 신규·업데이트 기능 300개 이상 전 세계 오너 배포. 리콜 98% OTA로 해결. 2) 리콜 OTA 해결은 서비스 비용 95% 절감이야. 딜러 방문 불필요. 소비자 경험 경쟁사 대비 압도야. 3) 차량 출고 후 기능이 늘어나는 건 자동차 업계 전례 없어. 보유 가치가 증가하는 자산. 애플 iPhone 모델이야. 4) 기능 개발→OTA 배포 사이클 빠른 테슬라가 항상 리드. 경쟁사는 이 체계 구축에 수년 걸려. 소프트웨어 플랫폼 해자. BUY.",
    likes: 1654,
    comments: 4,
  ...
  },
  {
    ...
    symbol: "TSLA",
    content:
      "Tesla 임팩트 리포트 '25 p.13 — Model Y 마일당 비용 $0.77 분석. 1) Model Y $0.77/마일. 경쟁 SUV 중 최저. Hyundai IONIQ 5 $0.85, Toyota RAV4 그 이상. Model Y가 10%+ 저렴한 공인 데이터야. 2) 마일당 비용은 연료비+유지보수 합산 TCO야. 공인 임팩트 리포트 데이터라 신뢰도 높아. 소비자 구매 결정 영향 줄 수 있어. 3) 10만 마일 타면 IONIQ 5 대비 $8,000 절감. 차량 수명 내 실질 절감액이 구매 결정 근거가 되는 수준이야. 4) 전기차 비싸다는 편견 깨는 공인 데이터. 대중 시장 침투 가속 근거야. BUY.",
    likes: 1123,
    comments: 3,
  ...
  },
  {
    ...
    symbol: "SPCX",
    content:
      "Raymond James SPCX Strong Buy, 목표가 $550 — 분석. 1) 등급: Strong Buy. 목표가: $550. 현재가 대비 상당한 상승 여력. NASDAQ 100 편입 모멘텀 반영했어. 2) 산정 배경: 패시브 자금 유입 + 기관 투자 자격 취득 + SpaceX 핵심 사업 성장성. Starlink·Falcon·Starship 세 개 성장 동력이야. 3) 레이먼드 제임스가 보수적 타겟 내는 기관인데 $550 낸 건 강한 확신이야. 데이터 기반 자신감 있는 숫자야. 4) 패시브 매수+기관 편입+애널 커버리지 삼박자 갖춰지면 수급 구조가 일방향이야. 단기 모멘텀 + 장기 성장성 동시 보유. STRONG BUY.",
    likes: 2456,
    comments: 6,
  ...
  },
  {
    ...
    symbol: "TSLA",
    content:
      "Cybercab 오스틴 북부 + 포트워스 + 달라스 동시 포착 — 텍사스 확장 분석. 1) 북부 오스틴 여러 운영 거점 + 포트워스 + 달라스 텍사스 번호판 동시 포착. 기가텍사스 로컬 배차가 최단 경로야. 2) DFW 메트로 = 미국 4위 대도시권. 달라스포트워스 공항 미국 2위 규모. 공항 픽업만으로도 수익 모델 완성이야. 3) 오스틴→포트워스→달라스 삼각지대 = 고속도로 연결 완벽. 장거리 이동 포함 완전한 운영 테스트 가능해. 4) 텍사스가 자율주행 규제 우호 주야. 완전 상용화 확정되면 다른 주 도미노 인허가 기대. 상용화 초읽기. BUY.",
    likes: 2876,
    comments: 7,
