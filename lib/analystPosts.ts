...
export const MOCK_ANALYST_POSTS: AnalystMockPost[] = [
  // ── 2026-06-26 신규 ──────────────────────────────────────────────────────
  {
    ...
    symbol: "SPCX",
    content:
      "SpaceX 공식 인포그래픽 채널 체크해봤어 — 7/6 NASDAQ-100 편입이 이제 공식 확정이야. 1) 448+ 로켓 발사·7,300+ 위성·$350B+ 기업가치가 공식 수치로 나왔어. 2) 인포그래픽 타임라인이 2002 창업부터 2026.7.6 편입까지 깔끔하게 정리됐어. 기관 IR 자료로 그대로 쓸 수 있는 퀄리티야. 3) QQQ AUM $320B × SPCX 비중 추정 → 수억 달러 기계적 매수 확정. 4) 패시브 자금 외에도 오늘 인포그래픽 나온 시점부터 기관 리서치 커버리지 확대될 거야. 공식 발표 = 기관 투자자 참여 허가 신호야.",
    likes: 567,
    comments: 3,
  ...
  },
  {
    ...
    symbol: "SPCX",
    content:
      "SpaceX T-Mobile 인수 시나리오 채널 체크해봤어 — $108B 딜이 현실적인 이유. 1) SpaceX 현금 $100.8B 보유 중. T-Mobile 시총 $108B이니까 현금만으로 커버 가능한 범위야. 2) Direct-to-Cell 완성: 위성이 실내·지하를 못 잡는 약점을 T-Mobile 셀타워가 보완해. 완전한 커버리지 완성이야. 3) T-Mobile 600MHz 저주파 대역 획득 = 농촌·건물 내부 Starlink 신호 강화. 4) 트럼프 행정부 빅테크 M&A 우호 환경. DOJ 통과하면 미국 통신 1위 자동 등극. 이 딜이 성사되면 SPCX 기업가치 재산정 필수야.",
    likes: 531,
    comments: 3,
  ...
  },
  {
    ...
    symbol: "SPCX",
    content:
      "SpaceX 천연가스 파이프라인 건설 채널 체크해봤어 — 수직통합의 완성이야. 1) Starship 연료가 액화 메탄(CH4)이야. 지금은 외부 공급업체한테 사는데 파이프라인 자체 건설하면 자급자족이 돼. 2) 비용 절감: 외부 공급업체 마진 제거. 발사당 연료비 수백만 달러 절감 가능해. 3) 발사 간격 단축: 연료 대기 시간 없애면 월 발사 횟수 2회 → 8회 목표 달성 속도가 빨라져. 4) Starlink V3 위성 배치 가속화 = 구독자 증가 속도 향상. 발사 비용 구조 변화가 SpaceX 마진을 근본적으로 개선해.",
    likes: 448,
    comments: 2,
  ...
  },
  {
    ...
    symbol: "SPCX",
    content:
      "SPCX Leopold +780% 채널 체크해봤어 — 기관 공개가 신호였던 이유. 1) 2025년 11월 $250에 $53M 공개됐어. 지금 $2,225+이면 7개월에 +780%야. 연환산 1,340%야. 2) Leopold Altschermer 공개가 왜 신호였나: 기관이 이 규모 공개할 때는 목표가 확인 후야. 먼저 조용히 사고 공개하는 구조거든. 3) 신규 목표가 $2,500 상향 — NASDAQ-100 편입 반영. 현재가 대비 +12.4% 추가 업사이드. 4) 기관 공개 직후 진입 전략이 가장 수익률이 높은 이유가 이 패턴이야.",
    likes: 489,
    comments: 2,
  ...
  },
  {
    ...
    symbol: "TSLA",
    content:
      "기가베를린 7,500대/주 + 1,000명 채용 채널 체크해봤어 — 유럽 수요 대응 완성이야. 1) 7,500대/주 × 52 = 연 390,000대. 유럽 Tesla 수요 소화에 딱 맞는 규모야. 2) 현지 생산 = EU 수입 관세 없음. 가격 경쟁력 + 마진 방어 동시야. 3) 5월 유럽 판매 2배 + 베를린 7,500대 목표 = Q3/Q4 출하량 서프라이즈 구조가 완성돼. 4) 채용 공고는 계획 확정의 증거야. 채용 없이 생산 늘어난다는 말은 없어. 1,000명 채용 = 실행 단계 진입.",
    likes: 467,
    comments: 2,
  ...
  },
  {
    ...
    symbol: "TSLA",
    content:
      "NHTSA AV 브레이크 페달 면제 채널 체크해봤어 — Cybercab 설계 혁명이야. 1) ADS 전용 차량에 수동 브레이크 페달 의무 공식 폐지야. AV Framework 5차 업데이트야. 2) Cybercab 영향: 페달 없는 구조로 설계 가능해지면 실내 공간 극대화 + 원가 절감이야. 승차 경험이 완전히 달라져. 3) 원가 절감 계산: 브레이크 페달 시스템 원가 약 $800~1,200/대. Cybercab 목표 생산 10만 대/년이면 연 $1억 절감이야. 4) 규제 장벽 제거 속도가 FSD·Robotaxi 확장 타임라인을 당기는 거야.",
    likes: 634,
    comments: 3,
  ...
  },
  {
    ...
    symbol: "TSLA",
    content:
      "Tesla 일본 수요 폭발 채널 체크해봤어 — 아시아 도미노 시작이야. 1) 배달 센터 포화 = 수요가 물류 처리 능력을 초과했다는 거야. 항구 직배송으로 전환한 것도 증거야. 2) 일본 = 세계 4위 자동차 시장. 여기서 Tesla 점유율 상승하면 아시아 FSD 수익화 도미노야. 3) 무료 슈퍼차징 + 보조금 조합이 사실상 가격 인하야. 엔화 약세 환경에서도 실질 구매 부담이 줄어드는 거야. 4) 일본 Q3 인도량이 폭발할 가능성 높아. 출하량 서프라이즈에 일본이 기여할 거야.",
    likes: 412,
    comments: 2,
  ...
  },
  {
    ...
    symbol: "MU",
    content:
      "Micron Q4 $50B 가이던스 채널 체크해봤어 — 반도체 역사상 최대 마진 구간 진입이야. 1) 매출 $50B × 마진 80% = 영업이익 $40B. 단일 분기 반도체 역대 최대야. 2) HBM3E 수요가 공급을 압도해. AI 서버 폭발 → 공급 부족 → 가격 결정력 Micron으로 이전. 이 구조는 1~2년 유지돼. 3) EPS·매출·가이던스 트리플 비트 = 월가 추정 즉시 상향. FY27 추정 올라가면 목표주가도 올라가. 4) TSLA Colossus + SPCX AI 데이터센터가 Micron HBM 수요야. AI 인프라 투자가 살아있다는 증거야.",
    likes: 598,
    comments: 3,
  ...
  },
  {
    ...
    symbol: "AVGO",
    content:
      "Broadcom AI $1T TAM 채널 체크해봤어 — 하이퍼스케일 맞춤 칩의 독점 구조. 1) CEO가 AI 자체 인프라 TAM $1T 발언. Google TPU·Meta MTIA 등 하이퍼스케일 커스텀 칩 설계를 AVGO가 독점에 가깝게 맡아. 2) XPU 설계 용역: 한번 계약하면 전환 비용이 엄청 커. 칩 아키텍처·소프트웨어·테이프아웃 전체가 AVGO에 의존하게 돼. 3) AVGO 점유율 30% 가정: $300B 매출 기회야. 현재 연매출 $50B 대비 6배야. 5~10년 성장 로드맵이야. 4) AI 인프라 테마에서 NVDA 다음으로 가장 확실한 수혜주야.",
    likes: 445,
    comments: 2,
  ...
  },
  {
    ...
    symbol: null,
    content:
      "매크로 GDP 2.1% + Cathie 경고 채널 체크해봤어 — 두 신호를 어떻게 읽어야 하나. 1) GDP 2.1% 최종치: 예상 1.6% 대폭 상회. 미국 경제가 예상보다 강해. 성장주에 유리한 환경이야. 2) AI 인프라 CapEx가 GDP 성장에 기여 중이야. Tesla·SpaceX·Micron 투자가 GDP에 잡히는 거야. 3) Cathie Wood 경고: 80~90년대 아시아 금융위기 패턴. 터키 등 신흥국 통화 방어 중. 달러 강세 + 금 상승 동반. 4) 두 신호 공존 = 미국 강 + 신흥국 약 구조. TSLA·SPCX·MU는 달러 표시 미국 자산이야. 신흥국 위기 때 안전자산 역할 가능해.",
    likes: 423,
    comments: 2,
...
export const MOCK_ANALYST_COMMENTS: Record<number, AnalystMockComment[]> = {
  ...
  [-173]: [
    ...
    { alias: "논현 황소 #52", content: "목표가 $2,500 상향이 NASDAQ-100 편입 반영이야. 편입 후 패시브 자금 유입 + 기관 리서치 확대가 추가되면 $2,500도 보수적일 수 있어. 다음 레벨 목표가 얼마나 올라가냐가 관건이야.", created_at: "2026-06-26T00:46:00.000Z" },
  ],
  [-174]: [
