...
import { fetchStooqUSBatch } from "@/lib/stooq";
...
export async function GET(req: NextRequest) {
  ...
  // Primary: Finnhub — live price (open) or prev-close fallback (closed)
  ...
  // Fallback: Stooq for any symbols Finnhub missed (rate-limited or no data)
  const missing = symbols.filter((s: string) => !map[s]);
  if (missing.length > 0) {
    const stooqMap = await fetchStooqUSBatch(missing);
    stooqMap.forEach((q, sym) => {
      map[sym] = { price: q.price, change: q.change, changePercent: q.changePercent };
    });
  }

  // Never cache an empty response — next request should retry
