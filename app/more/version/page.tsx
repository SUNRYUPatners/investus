...
const VERSIONS = [
  {
    ...
    current: true,
    items: [
      "홈탭 보유종목 즉시 표시 — 탭 이동 시 딜레이 없이 바로 로딩",
      "홈탭 포트폴리오 AI 분석 — 장마감 후 자동 분석·캐시, 장중 3회 무료 재분석",
      "자산 AI 비서 답변 개선 — 응답이 중간에 잘리는 문제 해결",
      "종목토론 댓글 — 팝업 방식 → 인라인 펼치기로 변경",
      "글 더보기 — 본문 탭·클릭으로 전체 펼치기 (종목토론·애널들은)",
      "애널들은 댓글 수 즉시 반영",
      "데스크탑 오른쪽 사이드바 너비 전 탭 통일 (340px)",
      "인사이트 탭 데스크탑 레이아웃 개선 (콘텐츠 짤림 해소)",
      "소개 페이지 법적 고지 추가 — KIFIN 유사투자자문업 사전교육 이수 표시",
      "자주 묻는 질문 업데이트 — 타 앱·증권사 시세 차이 설명 추가",
    ],
  },
  {
    ver: "v1.0.6",
